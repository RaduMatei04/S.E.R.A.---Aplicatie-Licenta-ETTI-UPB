<#-- Based on Keycloak 26.5.2 base/login/login.ftl. Layout and flow pages stay inherited. -->
<#import "template.ftl" as layout>
<#import "passkeys.ftl" as passkeys>
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    <#if section = "header">
        <span class="sera-login-heading">
            <span class="sera-login-title">${msg("loginAccountTitle")}</span>
            <span class="sera-login-welcome">${msg("seraLoginWelcome")}</span>
        </span>
    <#elseif section = "form">
        <div id="kc-form">
                        ...
                        <div class="${properties.kcFormGroupClass!} ${properties.kcFormSettingClass!}">
                            <div id="kc-form-options">
                                <#if realm.rememberMe && !usernameHidden??>
                                    <label class="sera-remember" for="rememberMe">
                    ...
                    <#list social.providers as p>
                        <li>
                            <a data-once-link data-disabled-class="${properties.kcFormSocialAccountListButtonDisabledClass!}" id="social-${p.alias}"
                               class="${properties.kcFormSocialAccountListButtonClass!} <#if social.providers?size gt 3>${properties.kcFormSocialAccountGridItem!}</#if>" href="${p.loginUrl}">
